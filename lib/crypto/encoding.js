...
**/
exports.encodeByMap = function encodeByMap(origin, options) {
    let _opts = Object.assign({
        bit: 5,
        map: 'jtzy_7qfkblovewangxum5dsrh86e3p9',
        cipher: 'khepri_crypto',
        mixed: s => s,
        join: '.'
    }, options);
    ...
    let _hex = origin, _str2 = '', key, code;
    let _last8Length, _last2Length, _keyOrigin = _opts.cipher;
    const _hexes_split8 = [], _arrBit = [];
    ...
    for (let j = 0; j < _str2.length; j += _opts.bit) {
        let _bit2 = _str2.substr(j, _opts.bit);
...
**/
exports.decodeByMap = function decodeByMap(code, options) {
    let _opts = Object.assign({
        bit: 5,
        map: 'jtzy_7qfkblovewangxum5dsrh86e3p9',
        remix: s => s,
    ...
    const { key, bit, map, remix, join, isHex } = _opts;
    let _keyOrigin, _codeOrigin, _last8Length, _last2Length, _sparr;
    if (typeof code === 'object' && code.key && code.code) {
        _keyOrigin = code.key;
        _codeOrigin = code.code;
    }
    else if (typeof code === 'string' && typeof join === 'string') {
        let ji = code.indexOf(join);
